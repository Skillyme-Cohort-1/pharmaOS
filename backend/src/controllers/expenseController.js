...
export async function getExpenseSummary(req, res, next) {
  try {
    ...
    const [todayResult, weekResult, monthResult, totalResult] = await Promise.all([
      prisma.expense.aggregate({ where: { date: { gte: todayStart } }, _sum: { amount: true }, _count: true }),
      prisma.expense.aggregate({ where: { date: { gte: weekStart } }, _sum: { amount: true }, _count: true }),
...
export async function updateExpense(req, res, next) {
  try {
    ...
    if (error.code === 'P2025') {
      return res.status(404).json({ success: false, error: 'Expense not found', code: 'NOT_FOUND' })
    }
    next(error)
  }
}
